...
特点卡：[[03-patterns/framework-characteristics/payload|Payload 特点]]
...
| `packages/ui` | Admin UI 和共享界面能力 |
| `packages/sdk` | SDK |
| `packages/graphql` | GraphQL 支持 |
| `packages/db-*` | MongoDB、Postgres、SQLite 等数据库适配 |
| `packages/plugin-*` | 表单、多租户、搜索、SEO、Stripe、MCP 等插件 |
| `templates` | blank、website、ecommerce、with-postgres 等模板 |
| `examples` | 示例项目 |

## 初步判断

Payload 的核心不是“已有数据库变后台”，而是“用 TypeScript 配置定义后台”。这和 [[02-reading-notes/directus--directus/monorepo-structure|Directus - Monorepo 结构]] 形成清晰对照：
...
- 框架特点：[[03-patterns/framework-characteristics/payload|Payload 特点]]
- Directus 对照：[[02-reading-notes/directus--directus/monorepo-structure|Directus - Monorepo 结构]]
